...
impl File {
    ...
    pub fn file_attr(&self) -> io::Result<FileAttr> {
        ...
        Ok(FileAttr { size: stat.size, file_type: opened_file_type(stat.file_type), mtime: stat.mtime })
    }
...
pub fn stat(path: &Path) -> io::Result<FileAttr> {
    ...
    if let Ok(fd) = syscall::open(path_bytes, OpenFlags::READ) {
        ...
        return Ok(FileAttr { size: st.size, file_type: opened_file_type(st.file_type), mtime: st.mtime });
    }
